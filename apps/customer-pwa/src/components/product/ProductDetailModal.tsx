      ...
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="pr-8">
            {isLoading ? t('common.loading') : product?.name}
          </DialogTitle>
          <Button
            variant="ghost"
            size="icon"
        ...
        {isLoading ? (
          <div className="flex items-center justify-center py-8">
            <div className="text-muted-foreground">{t('product.loadingDetails')}</div>
          </div>
        ) : product ? (() => {
          // Helper functions and calculations scoped to product context
          const variantGroup = product.product_modifier_groups?.find(
            (g: ProductModifierGroup) => g.selection_type === 'single'
          );
          const extrasGroups = product.product_modifier_groups?.filter(
            (g: ProductModifierGroup) => g.selection_type === 'multiple'
          ) || [];
          ...
          const calculateTotalPrice = () => {
            ...
            if (selectedVariantId && variantGroup) {
              const selectedVariant = variantGroup.product_modifiers.find(
                (m: ProductModifier) => m.id === selectedVariantId
              );
              if (selectedVariant) {
          ...
          const handleAddToCart = () => {
            ...
            if (selectedVariantId && variantGroup) {
              ...
              if (selectedVariant) {
                optionsPrice += selectedVariant.additional_price;
              }
            }
            ...
            extrasGroups.forEach((group: ProductModifierGroup) => {
              group.product_modifiers.forEach((modifier: ProductModifier) => {
                if (selectedExtras.has(modifier.id)) {
                  optionsPrice += modifier.additional_price;
                }
              });
            });

            const selectedOptionsDescription = [
              selectedVariantId && variantGroup?.product_modifiers.find(
                m => m.id === selectedVariantId
              )?.name,
              ...Array.from(selectedExtras).map(extraId => {
                const group = extrasGroups.find(
                  g => g.product_modifiers.some(m => m.id === extraId)
                );
                const modifier = group?.product_modifiers.find(
                  m => m.id === extraId
                );
