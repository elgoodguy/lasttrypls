...
const StoreCard = React.forwardRef<HTMLDivElement, StoreCardProps>(
  ({ className, store, ...props }, ref) => {
    ...
    return (
      <Card
        ref={ref}
        className={cn("w-[280px] overflow-hidden cursor-pointer hover:shadow-md transition-shadow", className)}
        {...props}
      >
        {/* Store Logo/Image */}
        <div className="h-32 bg-muted flex items-center justify-center">
          {store.logo_url ? (
            <img src={store.logo_url} alt={`${store.name} logo`} className="h-full w-full object-contain p-2" />
          ) : (
